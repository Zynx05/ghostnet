...
MEMBERS = [
    ...
    # ------------------------------------------------------------------ 2
    {
        ...
        "problem": (
            "We started on SQLite and moved to PostgreSQL. Every placeholder "
            "changed from a question mark to percent s. PostgreSQL has no "
            "lastrowid, so every insert that needed an id got a RETURNING "
            "clause. The old startup hook was also removed from FastAPI, so it "
            "became a lifespan handler."
        ),
        "qa": [
