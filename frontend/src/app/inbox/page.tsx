...
function Inbox() {
  ...
  useEffect(() => {
    api.inbox().then(setRows).catch(e => { setFlash({ text: e.message, err: true }); setRows([]); });
  }, []);
  ...
  return (
    <div className="page">
      <section className="card hero">
        ...
        <div className="card-meta">Everything comes to you in writing. Nobody can call you. A tap means a company wants to talk, and you decide whether to answer.</div>
      </section>
      <Flash message={flash} />
      {rows === null && <div className="card"><Skeleton rows={3} /></div>}
      {rows !== null && rows.length === 0 && (
        <div className="card"><EmptyState title="Nothing yet" description="Enter a challenge. Taps and feedback land here." /></div>
      )}
      {(rows ?? []).map(m => (
        <section key={m.id} className="card">
          <div className="row-between">
            <div className="card-meta"><Link href={`/challenge/${m.challenge_id}`} className="link">{m.title}</Link> · {m.company}</div>
